...
const ICONS = {
  ...
  moon: `<svg viewBox="0 0 24 24" width="22" height="22"><path d="M12 3a9 9 0 1 0 0 18c-3-1.8-5-5.2-5-9s2-7.2 5-9Z" fill="currentColor"/></svg>`,
...
async function handleSignup(e) {
  ...
  const signupResult = await Api.signup(profile);
  if (!signupResult.ok && !signupResult.offline) {
    console.warn("Backend signup failed — this account will only work on this device until it's fixed:", signupResult.error);
  }
  state.user = profile;
  saveUser();
  state.navStack = [];
...
function screenDhikrMain() {
  ...
  const rows = ids.map(id => {
    ...
    const countDisplay = state.lang === "bn" ? toBnNumber(count) : count;
    return `
      <button class="dhikr-list-row" onclick="setScreen('dhikrDetail','${id}')">
        <span>${DHIKR_LIBRARY[id][state.lang].name}</span>
        <span class="dhikr-count-bubble">${countDisplay}</span>
      </button>`;
  }).join("");
  return `
    <div class="screen">
...
function openAddDhikr() {
  ...
  if (!options.length) { panel.innerHTML = `<p class="muted">${state.lang === "bn" ? "আর কিছু নেই।" : "Nothing more to add."}</p>`; return; }
  panel.innerHTML = `
    <div class="add-panel">
  ...
  return `
    <div class="screen">
      ...
      <div class="tap-row">
        ...
        <button class="tap-btn" onclick="tapDhikr('${id}',33)">33</button>
